...
class KubernetesListener {
  ...
  exit_deploymentSpec(deploymentSpecNode) {
    ...
    if (templateNode && Object.keys(templateNode.value).length) {
      const podComponent = this.createComponentFromTree(templateNode, 'v1', 'Pod');
    ...
    // prevent exit_root from visiting this node again
    delete deploymentSpecNode.value.template;
    // this selector is automatically generated from the Pod template labels, so we don't need to parse it
    delete deploymentSpecNode.value.selector;
  }
  ...
  exit_podSpec(podSpecNode) {
    ...
    ['initContainers', 'containers'].forEach((k8sContainerAttributeName) => {
      const k8sContainerComponents = podSpecNode.value[k8sContainerAttributeName]?.value.map(
        (containerNode) => {
          const volumeMountComponents = this.createVolumeMountComponentsFromTree(containerNode, podSpecNode);
          const k8sContainerComponent = this.createComponentFromTree(containerNode, 'others', 'Container');
  ...
  createVolumeMountComponentsFromTree(containerNode, podSpecNode) {
    ...
    containerNode.value.volumeMounts?.value.forEach((volumeMountNode) => {
      ...
      const volumeComponent = this.createComponentFromTree(volumeMountNode, 'others', volumeKind);
      volumeComponent.attributes.push(
        ...this.createAttributesFromTreeNode(volumeNode, volumeComponent.definition).filter(({ name }) => name !== 'name'),
      );
  ...
  exit_cronJobSpec(cronJobSpecNode) {
    ...
    if (jobTemplateNode && Object.keys(jobTemplateNode.value).length) {
      const jobComponent = this.createComponentFromTree(jobTemplateNode, 'batch/v1', 'Job');
  ...
  createComponentFromTree(node, apiVersion, kind) {
    const definition = this.definitions.find((definition) => definition.apiVersion === apiVersion && definition.type === kind);
